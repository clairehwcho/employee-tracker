...
class Employee {
    ...
    showEmployeesByDepartment () {
        const sql = 'SELECT department.id, department.name AS department, COUNT(employee.id) AS total_num_of_employee, GROUP_CONCAT(DISTINCT CONCAT(employee.first_name, \' \', employee.last_name) ORDER BY employee.first_name SEPARATOR \', \') AS employees From employee LEFT JOIN role ON role.id = employee.role_id LEFT JOIN department ON department.id = role.department_id LEFT Join employee AS manager ON manager.id = employee.manager_id GROUP BY department.name ORDER BY department.id ASC;'
